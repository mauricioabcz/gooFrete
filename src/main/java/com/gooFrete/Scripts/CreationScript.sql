...
CREATE TABLE [emit].[TravelStop](
	[Id] [uniqueidentifier] NOT NULL,
	[Praca] [varchar](50) NULL,
	[Valor] [decimal](18, 2) NULL,
	[TravelId] [uniqueidentifier] NULL,
	[InsertDate] [datetime] NOT NULL,
 CONSTRAINT [PK_TravelStop] PRIMARY KEY CLUSTERED 
(
	[Id] ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON, OPTIMIZE_FOR_SEQUENTIAL_KEY = OFF) ON [PRIMARY]
) ON [PRIMARY]
...
ALTER TABLE [emit].[TravelStop]  WITH CHECK ADD  CONSTRAINT [FK_TravelStop_Travel] FOREIGN KEY([TravelId])
REFERENCES [emit].[Travel] ([Id])
GO

ALTER TABLE [emit].[TravelStop] CHECK CONSTRAINT [FK_TravelStop_Travel]
GO
